...
LastEditTime: 2025-04-03 12:59:43
...
def main():
    if os.path.exists(settings.pkl_path):
        ...
        different = montage.image_dir != settings.image_dir or montage.block_height != settings.block_height or montage.block_width != settings.block_width or montage.fix_ratio != settings.fix_ratio or montage.lazy_load != settings.lazy_load
        if different:
            montage: Montage = Montage(image_dir=settings.image_dir, block_height=settings.block_height, block_width=settings.block_width, fix_ratio=settings.fix_ratio, lazy_load=settings.lazy_load)
